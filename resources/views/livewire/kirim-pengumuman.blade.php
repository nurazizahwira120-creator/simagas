...
<div class="space-y-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">Tulis Pengumuman</h2>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                Pengumuman muncul di lonceng notifikasi penerima. Pengiriman WhatsApp opsional.
            </p>
        </div>

        <form wire:submit="kirim" class="space-y-5 p-6">
            ...
            <div class="flex flex-wrap items-center gap-3 pt-1">
                {{-- Konfirmasi hanya dipasang saat WhatsApp dicentang.
                     ...
                     Memakai data-konfirmasi (SweetAlert2), bukan wire:confirm:
                     wire:confirm memanggil confirm() bawaan browser dan tidak
                     bisa diarahkan ke SweetAlert2. --}}
                <button type="submit" wire:loading.attr="disabled" wire:target="kirim"
                    @if ($kirim_wa && $this->gatewayAktif)
                    ...
                    <svg wire:loading wire:target="kirim" class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
                    </svg>

                    <span wire:loading.remove wire:target="kirim">Kirim Pengumuman</span>
                    <span wire:loading wire:target="kirim">Mengirim…</span>
                </button>

                <p class="text-xs text-gray-500 dark:text-gray-400">
                    Pengumuman tidak bisa ditarik kembali setelah terkirim.
                </p>
            </div>
        </form>
    </div>
